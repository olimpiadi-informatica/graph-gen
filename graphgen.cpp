...
extern "C"
{
	...
	static int RS_init(
		...
		if (!PyArg_ParseTuple(args, "iii", &num, &min, &max))
			return -1;
		if (self->rs)
		{
	...
	static PyObject* DisjointSet_merge (
		...
		if (!PyArg_ParseTuple(args, "ii", &a, &b))
			return NULL;
		if (a<0 || a>=self->disjoint_set->size() || b<0 || b>=self->disjoint_set->size())
		{
			PyErr_SetString(PyExc_ValueError, "Values out of range!");
			return NULL;
		}
		self->disjoint_set->merge(a, b);
		Py_RETURN_NONE;
	}
	...
	static PyObject* UG_add_random_edges (
		UGObj* self,
		PyObject *args,
		PyObject *kwds
	) {
	...
	static PyObject* DG_add_random_edges (
		DGObj* self,
		PyObject *args,
		PyObject *kwds
	) {
	...
	PyMODINIT_FUNC initgraphgen(void)
	{
		...
		PyModule_AddObject(m, "DSU", (PyObject *)&DisjointSetType);
